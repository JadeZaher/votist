<script lang="ts">
	...
	let filteredPosts = $derived(($searchTerm: string, $selectedCategory: string) => {
			...
			<div class="card-body flex h-100 max-w-screen-sm flex-col p-0 md:gap-3">
				...
				<div class="mt-6 flex flex-row justify-between gap-6 text-lg">
					...
					<p class="text-gray-400">
						{firstPost.readingTime?.split(' ')[0] || 'N/A'} Min Read
					</p>
				</div>
			</div>
		</div>
...
{:else}
	<section>
		...
		>
			<div class="card-body flex h-100 max-w-screen-sm flex-col p-0 md:gap-3">
				<h1 class="text-secondary-green align-self-start text-lg font-bold lg:text-2xl">
					RESEARCH
				</h1>
				<h2 class="card-title py-1 text-3xl lg:text-4xl lg:leading-[3.2rem]">
					Content Coming Soon
				</h2>
				<h3 class="card-subtitle text-muted mb-2">
					We are currently setting up our research content. Please check back soon for the latest
	...
	<div class="w-screen-2xl mx-auto flex flex-wrap justify-center gap-12 p-8 xl:w-5/6">
				...
				<a href={`research/${wpPost.slug || '#'}`}>
					<div class="card h-full max-w-[84vw] grow p-5 shadow-lg lg:max-w-[24vw]">
						<img
							...
							src={wpPost.imageUrl || '/placeholder-research.jpg'}
							alt=""
						/>
						<div class="flex h-full flex-col justify-between pt-2">
							<div class="flex flex-row text-xs text-gray-400">
								...
								<p>{@html wpPost.tag || 'General'}</p>
							</div>
							<h3 class="mt-3 text-lg font-bold">{@html wpPost.title || 'Untitled'}</h3>
							<div class="mt-6 flex flex-row justify-between text-lg">
								<h4 class="text-secondary-green font-bold">READ ARTICLE</h4>
								<p class="text-gray-400">
									{wpPost.readingTime?.split(' ')[0] || 'N/A'} Min Read
								</p>
							</div>
						</div>
					</div>
		...
		{:else}
			<div class="w-full text-center py-12">
				<p class="text-gray-500 text-lg">No research posts available at the moment.</p>
				<p class="text-gray-400 text-sm mt-2">Please check back later for new content.</p>
			</div>
		{/each}
	</div>
	<div
