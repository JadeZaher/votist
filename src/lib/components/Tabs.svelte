  ...
  {#each tabs as tab (tab.id)}
    <div class="tab-wrap" style:width={tabWidth}>
      <a 
        role="tab"
        href={tab.href} 
        class="tab relative h-[49px] flex items-center pl-6 pr-8 gap-2 transition-colors"
      ...
      >
        {#if tab.icon}
          <div 
            class="w-6 h-6" 
            style:color={$page.url.pathname === tab.href ? 'var(--color-base-100)' : 'var(--color-base-content)'}
          >
...
<style>
  ...
  .tab {
    clip-path: path('M0 14v39h254c-3.288-1.684-5-7.5-19-25-12.963-16.2033-20-24-31.553-24H10c-5.52285 0-10 4.4772-10 10z');
    position: relative;
  }
